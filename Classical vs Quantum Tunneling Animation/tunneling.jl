...
# ╔═╡ 5fa6f860-5de5-11ed-2b63-2d385bc02bde
using DifferentialEquations, LinearAlgebra, Plots, StatsPlots, LaTeXStrings, PlotThemes, PlutoUI; theme(:dao)

# ╔═╡ fc314305-e9a7-445e-87f1-33d4d5a29512
md"
## Classical vs. quantum tunneling
#### CAS PY 452, Fall 2022

*Emmy Blumenthal*
"

# ╔═╡ 49b5d08a-0f7f-47c5-b0bb-3cf7013091b2
...
# ╔═╡ cdb050e3-b2cc-4693-a4a7-4f17cb9cb2ec
plot(x/σᵥ,Vx/V₁,label=L"V",xlim=(-20,20),xlabel=L"x/\sigma_V",ylabel=L"V / V_0")

# ╔═╡ 45feb740-9325-445f-8468-ec95f36d05c4
md"
#### Classical simulations:
"

# ╔═╡ 89e527be-572b-4563-89b4-77b941a61d8c
 # Newton's second law
...
# ╔═╡ e6984637-72f2-4dfe-9c34-c4655e16f080
# Get a solution given initial position and momentum
function trajec(x0,p0)
    u0 = [x0,p0/m];
    tspan = (0.0,1000.0);
    prob = ODEProblem(eom!,u0,tspan);
    sol = solve(prob,dtmax=1,saveat=4,reltol=1e-8,abstol=1e-8); 
    return sol
end;

# ╔═╡ a8aa688d-e666-49bb-802e-4dab63254f94
# Choose parameters for initial ensemble distribution
x₀ = -100; σₓ₀ = 3.0; p₀ = 0.26; σₚ₀ = ħ/(2σₓ₀);

# ╔═╡ c7a9a5ed-803c-45e6-901f-c779f158beec
begin #solve differential equations for 1000 particles
	ensemblepstemp = []; ensemblexstemp = []; tsclassical = 0:2:1000;
	for _ in 1:2000
		let p = p₀ + σₚ₀*randn(), x = x₀ + σₓ₀*randn()
			atrajec = trajec(x,p);
			push!(ensemblexstemp,atrajec(tsclassical,idxs=1).u)
			push!(ensemblepstemp,atrajec(tsclassical,idxs=2).u)
		end
	end
	ensemblexs = hcat(ensemblexstemp...)
	ensembleps = hcat(ensemblepstemp...)
end;

# ╔═╡ 12c81a6a-453b-4937-86cb-7fd819ffcef2
@bind t Clock(0.2)

# ╔═╡ 5e060e47-2c6c-48f2-b3e4-a8b17af24a82
plottclassical = mod(10t,1000);

# ╔═╡ b4e05ce5-c7e5-4638-a4eb-ae729cee15ab
# @bind plottclassical html"""
# <input type=range min=0 max=1000 step=10 value=0 style='width: 100%;' oninput='this.nextElementSibling.value=this.value;'>
# """

# ╔═╡ 5c78a962-3b27-446e-ae39-7019b807f86a
begin
	histogram(ensemblexs[first(eachindex(tsclassical)[tsclassical .== plottclassical]),:],bins=-200:2.5:200,label=:none,legend=:topright)
	plot!(xlim=(-200,200),xlabel="Position",ylabel="Classical particle counts")
end

# ╔═╡ d3843c64-b199-448a-ab9c-80a331c1d5e9
begin
	histogram(ensembleps[first(eachindex(tsclassical)[tsclassical .== plottclassical]),:],label=:none,bins=-1:0.02:1)
	vline!([sqrt(2*m*V₁)],label=L"\sqrt{2m V_{\mathrm{max}}}",legend=:topright)
	plot!(xlim=(-1,1),xlabel="Momentum",ylabel="Classical particle counts")
end

# ╔═╡ c3c15138-083f-44d8-bd7d-720b1b06dd72
begin
	histogram2d(ensemblexs[first(eachindex(tsclassical)[tsclassical .== plottclassical]),:],ensembleps[first(eachindex(tsclassical)[tsclassical .== plottclassical]),:],xlim=(-200,200),ylim=(-0.6,0.6),bins=(-200:4:200,-0.6:0.02:0.6))
	hline!([sqrt(2*m*V₁)],label=L"p = \sqrt{2mV_{\mathrm{max}}}",legend=:topright)
	plot!(xlabel=L"x",ylabel=L"p")
end

# ╔═╡ ab9c1d3c-11b0-4b27-9d68-cfb0f24e9f55
md"
Observe that a few particles do get through... They are exactly those that have an initial momentum greater than $\sqrt{2mV_\text{max}}$
"

# ╔═╡ 07d820b7-c4db-4791-a832-59e17d4a891b
md"
#### Quantum simulations
"

# ╔═╡ 57a12061-f62d-4d67-a370-59c57b935211
...
# ╔═╡ aac1a1a5-9ef4-41f0-b432-ab1f37e9dc46
# compute energies and stationary states:
begin
	# ℰs, sψs = eigen(H)
	eigs = eigen(H);
	ℰs = eigs.values;
	sψs = eigs.vectors .* Δx^(-1/2);
	eigfrees = eigen(H₀);
	ℰfrees = eigfrees.values;
	sψfrees = eigfrees.vectors .* Δx^(-1/2);
end;

# ╔═╡ 962409d0-1104-4551-95c2-f91dd8bab49b
# initial wave function
ψ₀ = normalize(exp.(-(x .- x₀).^2 ./(4 * σₓ₀^2) + 1.0im * p₀ * x /ħ)) * Δx^(-1/2);

# ╔═╡ 4e975d6c-d61d-4d18-8a00-9bde5730ae75
# time evolution by attaching phase factors
begin
	ψ₀eigenbasis = sψs' * ψ₀; # convert to stationary state basis
	ψ₀freeeigenbasis = sψfrees' * ψ₀;
	ψ(t) = sψs * Diagonal(exp.(-1.0im * t*ℰs/ħ )) * ψ₀eigenbasis * Δx;
	ψfree(t) = sψfrees * Diagonal(exp.(-1.0im * t*ℰfrees/ħ ))  *ψ₀freeeigenbasis * Δx;
end;

# ╔═╡ 2f729dda-280a-43af-abfb-3f492f1bc1e4
begin # create vector of time-evolved states
	tmax = 1000; tstep=2.5;
	ψs = ψ.(0:tstep:tmax)
	PDFs = (X->abs.(X).^2).(ψs)
end;

# ╔═╡ 09d27e70-c704-4f14-9517-4f551f334a68
begin # same for free states
	ψfrees = ψfree.(0:tstep:tmax)
	PDFfrees = (X->abs.(X).^2).(ψfrees)
end;

# ╔═╡ 8720acd6-58bc-4739-9e1a-7c82bf949aae
begin # create a momentum basis
	k = (-2π/Δx ):(2π/(L/2 + Δx/2)):(2π/Δx);
	pBasis = hcat(Δx^(-1/2)*normalize.(eachcol(exp.(1.0im * x * k' /ħ)))...);
end;

# ╔═╡ c444f531-c7e1-4ec4-9aa8-c7dd3964a5f1
begin # evolution in momentum basis
	moms = (X->abs.(pBasis' * X).^2).(ψs);
	momsfree = (X->abs.(pBasis' * X).^2).(ψfrees);
end;

# ╔═╡ 3fc7a6cf-11bb-40c3-9878-d251717537ba
# @bind ploti Slider(eachindex(PDFs))
# @bind plott html"""
# <input type=range min=0 max=1000 step=2.5 value=0 style='width: 100%;' oninput='this.nextElementSibling.value=this.value;'>
# """

# ╔═╡ 78c92409-3445-4497-a76e-fbe502f571fd
@bind tt Clock(0.2/4)


# ╔═╡ 713f2934-c683-4c50-8618-0ced4355b935
plott = mod(2.5tt,1000)

# ╔═╡ 87ae5643-d0eb-461f-8d9f-3524aadbaf52
begin
	plot(x[begin:2:end],PDFs[first(eachindex(0:tstep:tmax)[0:tstep:tmax .== plott])][begin:2:end],ylim=(-0.001,0.04),xlim=(-200,200),label=L"|\psi|^2")
	plot!(x[begin:2:end],PDFfrees[first(eachindex(0:tstep:tmax)[0:tstep:tmax .== plott])][begin:2:end],label=L"|\psi_{\mathrm{free}}|^2")
	plot!(x[begin:2:end],Vx[begin:2:end]/2,label=L"V")
	plot!(xlabel=L"x",ylabel="Probability density",size=(650,400),dpi=300)
end

# ╔═╡ e6f665da-884b-446f-90dc-5cb4bc24c325
begin
	plot(k,moms[first(eachindex(0:tstep:tmax)[0:tstep:tmax .== plott])],label=L"|\phi|^2")
	plot!(k,momsfree[first(eachindex(0:tstep:tmax)[0:tstep:tmax .== plott])],label=L"|\phi_{\mathrm{free}}|^2")
	plot!(xlim=(-1,1),ylim=(-.005,0.3))
	vline!([sqrt(2*m*V₁)],label=L"\sqrt{2mV_{\mathrm{max}}}")
	plot!(xlabel=L"p",ylabel="Probability density",dpi=300,size=(600,400))
end

# ╔═╡ b5804bf8-926d-42ad-86a4-7de266f078cc
md"
We see that there is a significant probability that at long times, there will be particles which pass the barrier and have momentum less than $\sqrt{2m V_{\mathrm{max}}}$! This is tunneling!
"

# ╔═╡ bc737be8-1518-48f3-ab62-595996dc8227
md"
We can see how the phase-space dsitribution evolves by using the Wigner quasi-probability distribution. There is a nice visualization on Wikipedia here [here](https://en.wikipedia.org/wiki/File:Wigner_function_for_tunnelling.ogv).
"
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "42ad442bf2d3ca694963591653b15d65d8af0f24"
...
git-tree-sha1 = "acba6ce3d8cfdefbddc84b6840568a75f802bbbc"
